...
@section('title', 'ODF Ports: ' . $odf->name)
@section('page-title', 'ODF Port Map')
...
<div class="row mb-3">
    <div class="col-md-6">
        <h4 class="fw-bold">{{ $odf->name }} - Port Map</h4>
        <p class="text-muted mb-0">
            <code>{{ $odf->code }}</code> |
            Total: {{ $odf->total_ports }} ports |
            Used: <span class="badge bg-warning">{{ $odf->used_ports }}</span> |
            Available: <span class="badge bg-success">{{ $odf->getAvailablePorts() }}</span>
        </p>
    </div>
    <div class="col-md-6 text-end">
        <a href="{{ route('odfs.show', $odf) }}" class="btn btn-secondary btn-sm">
    ...
    <div class="card-body">
        <div class="row align-items-center">
            <div class="col-lg-6 mb-3 mb-lg-0">
                <div class="mb-2">
                    <strong>Port Utilization:</strong> {{ $odf->getUsagePercentage() }}%
                </div>
                <div class="progress" style="height: 30px;">
                    <div class="progress-bar {{ $odf->getUsagePercentage() >= 80 ? 'bg-danger' : ($odf->getUsagePercentage() >= 60 ? 'bg-warning' : 'bg-success') }}"
                         style="width: {{ $odf->getUsagePercentage() }}%">
                        {{ $odf->used_ports }}/{{ $odf->total_ports }} ports used
                    </div>
                </div>
            </div>
            <div class="col-lg-6">
        ...
        <div class="row align-items-center">
            <div class="col-md-6">
                <h6 class="fw-bold mb-0">
                    <i class="bi bi-grid-3x3"></i> Visual Port Map ({{ $odf->total_ports }} Ports)
                </h6>
            </div>
...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <span class="badge bg-info">{{ $ports->where('status', 'used')->count() }} connected ports</span>
        </div>
    </div>
    <div class="card-body">
            ...
            <table class="table table-sm table-hover align-middle">
                <thead class="table-light">
                    <tr>
                        ...
                        <th width="25%">Cable</th>
                        <th width="12%">Type</th>
                        <th width="25%">Destination</th>
                        <th width="12%">Connector</th>
                        <th width="8%">Actions</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($ports->where('status', 'used') as $port)
                        <tr>
                            <td>
                            ...
                            <td>
                                @if($port['cable'])
                                    <span class="badge bg-secondary">{{ ucfirst($port['cable']['cable_type']) }}</span>
                                @else
                                    -
                                @endif
                            </td>
                            <td>
                                @if($port['cable'] && $port['cable']['end_point'])
                                    <span class="badge bg-info">
                                        {{ strtoupper($port['cable']['end_point']['type']) }}
                                    </span>
                                    {{ $port['cable']['end_point']['name'] }}
                                @else
                                    <span class="text-muted">-</span>
                                @endif
                            </td>
                            <td>
                                @if($port['cable'] && $port['cable']['start_connector_type'])
                                    <span class="badge bg-dark">{{ $port['cable']['start_connector_type'] }}</span>
                                @else
                                    <span class="text-muted">-</span>
                                @endif
                            </td>
                            <td>
                                @if($port['cable'])
                                    <a href="{{ route('cable-segments.show', $port['cable']['id']) }}"
                                       class="btn btn-outline-primary btn-sm" title="View Cable">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                @endif
                            </td>
...
@if($ports->where('status', 'available')->count() > 0)
<div class="card border-0 shadow-sm mt-3">
    <div class="card-header bg-white">
        <h6 class="fw-bold mb-0">
            <i class="bi bi-check-circle"></i> Available Ports
            <span class="badge bg-success">{{ $ports->where('status', 'available')->count() }}</span>
        </h6>
    </div>
    <div class="card-body">
        <div class="d-flex flex-wrap gap-2">
            @foreach($ports->where('status', 'available')->take(60) as $port)
                <span class="badge bg-success">{{ $port['number'] }}</span>
            @endforeach
            @if($ports->where('status', 'available')->count() > 60)
                <span class="badge bg-secondary">+ {{ $ports->where('status', 'available')->count() - 60 }} more</span>
            @endif
        </div>
    </div>
</div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const savedLayout = localStorage.getItem('odf_ports_layout');
        if (savedLayout) {
    ...
    function changeLayout(cols) {
        ...
        // Save preference
        localStorage.setItem('odf_ports_layout', cols);
    ...
    // Port click event (optional - untuk future enhancement)
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('.port-item').forEach(function(portItem) {
            portItem.addEventListener('click', function() {
                ...
                // Future: Bisa ditambahkan modal atau detail popup disini
                // showPortDetailModal(portNumber);
            });
        });
    });
